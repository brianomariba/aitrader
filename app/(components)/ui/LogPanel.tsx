  ...
  return (
    <div className="mt-2">
      <div className="mb-1 flex items-center justify-between">
        <h3 className="text-sm font-medium opacity-80">Console</h3>
      ...
      <div className="h-48 overflow-auto rounded-lg bg-black/30 p-2 text-xs">
        {log.length === 0 ? <div className="opacity-50">No logs yet…</div> : log.map((l, i) => <div key={i} className="whitespace-pre">{l}</div>)}
      </div>
    </div>
  )
}
